...
Aetherfall은 몰락한 왕국을 탐험하며 근접 전투와 검기, 상호작용, 체크포인트 진행을 연결하는 3인칭 액션 RPG 프로토타입이다. 20~30분 분량의 버티컬 슬라이스를 목표로 한다. 이 문서는 게임 흐름과 계층별 책임, 주요 시스템의 연결을 설명한다.

개발자와 Codex가 소스코드를 공동 작성한 개인 프로젝트다.
...
흐름의 조정자는 [AetherGameModeBase](https://github.com/KangWooKim/Aetherfall/blob/2dfcf0cd57cdd7972c35c6d25d4c38c22fcd6ae9/Source/Aetherfall/Private/AetherGameModeBase.cpp#L1)다. 진행 상태는 수집·개방·완료 라벨 집합과 체크포인트 상태로 보관한다. 상호작용 액터는 자신의 기능을 실행한 후 GameMode의 진행 API에 결과를 알린다. 저장은 체크포인트·조우 완료·목표 완료 등 명시적인 시점에서 이루어진다.
...
GameInstanceSubsystem은 맵을 전환하는 동안 유지할 서비스 상태를, WorldSubsystem은 현재 월드에 속한 풀을 담당한다. Character의 컴포넌트는 소유 액터와 함께 구성된다. GameMode는 현재 월드의 조우·진행·저장 시점과 재시도를 조정한다.
...
Character가 개별 기능을 기본 서브오브젝트로 생성한다. 입력 바인딩은 Controller가 담당하고, Character와 각 Component의 API를 호출해 기능을 실행한다. Combat은 소유 Character와 Health 등 전투에 필요한 상태를 참조한다.

## 전투의 처리 경로

`입력 → 행동 허용 검사 → 자원/타이머/상태 계획 → 몽타주·스윕 실행 → 대상 선택 → 피해 적용 → 피드백`

ActionGate·ActionState·ActionTimer·ActionExecution·ActionTuning·Resource 정책은 현재 조건에서 필요한 판단이나 변경 계획을 만든다. CombatComponent는 결과를 읽고 실제 상태·타이머·애니메이션을 조정한다. TracePolicy는 스윕 파라미터를 계산하고, DamagePolicy는 대상과 체력 상태를 확인해 피해를 적용한다.
...
| 조정 지점 | 역할 | 데이터 적용 방식 |
| --- | --- | --- |
| [AetherCombatActionDataAsset.h](https://github.com/KangWooKim/Aetherfall/blob/2dfcf0cd57cdd7972c35c6d25d4c38c22fcd6ae9/Source/Aetherfall/Public/AetherCombatActionDataAsset.h#L1) | 공격 수치·몽타주·효과의 선택적 오버라이드 | 오버라이드 플래그에 따라 값을 선택하고 빈 참조에는 fallback을 적용 |
...
| [AetherDialogueDataAsset.h](https://github.com/KangWooKim/Aetherfall/blob/2dfcf0cd57cdd7972c35c6d25d4c38c22fcd6ae9/Source/Aetherfall/Public/AetherDialogueDataAsset.h#L1) | 대화 줄과 재생 설정 | 시간 기반 Mock TTS로 대화 재생 시간을 구성 |
...
Source는 게임 모듈과 Game/Editor Target으로 구성된다. 개발 환경은 UE 5.4 / Visual Studio 2022 MSVC 14.34 / Windows SDK 10.0.22000이다. 실행 프로젝트의 `.uproject`, Config, 맵·Blueprint·애니메이션·외부 에셋은 별도로 관리한다.

## 외부 자산과 작성 범위

핵심 게임플레이 C++ 코드는 개발자와 Codex가 공동 작성했다. 외부 아트·애니메이션·VFX·음향은 Marketplace/Fab 자산을 활용했다. 자산 사용 방침에는 Dark Knight Male/Female, Great Sword, InfinityBladeEffects, FantasyOrchestral이 기록되어 있다.
