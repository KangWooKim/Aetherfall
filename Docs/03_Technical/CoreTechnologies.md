...
현재 코드와 실행 조건을 연결할 수 있는 정량 실행 성능 결과는 없습니다. 대표 전투의 프레임·게임 스레드·메모리 비용, 검기 반복 발사의 생성·할당 비용, 저장·복원·로딩 지연은 미측정입니다. 과거 CSV는 대상 버전과 유효 측정 구간을 확정할 수 없어 결과로 인용하지 않습니다.

현재 코드에 대응하는 실행 파일을 확보해야 새 계측을 진행할 수 있습니다. 공개본에는 Source와 문서가 있고 실행에 필요한 .uproject·Config·Content가 포함되지 않아 단독 실행도 재현할 수 없습니다. 풀 보관 수, 화면의 체감 속도, 루트 플레이 시간과 설정 확인 기한으로 성능 개선율을 계산하지 않습니다.
